...
#if NONCONS==1 /*Powell*/
          phi_L(IRHOU:IRHOW)=U_master(IB1:IB3,p,q,SideID)
          phi_L(IB1:IB3)=U_master(IRHOU:IRHOW,p,q,SideID)/U_master(IRHO1,p,q,SideID)
          phi_L(IRHOE)  =SUM(phi_L(IRHOU:IRHOW)*phi_L(IB1:IB3))
          Flux(IRHOU:IB3,p,q,SideID)=Flux(IRHOU:IB3,p,q,SideID)-0.5*phi_L(IRHOU:IB3)*PrimL(IB1)   !1/2 phi B_n
#elif NONCONS==2 /*Brackbill*/
          phi_L(IRHOU:IRHOW)=U_master(IB1:IB3,p,q,SideID)
          Flux(IRHOU:IRHOW,p,q,SideID)=Flux(IRHOU:IRHOW,p,q,SideID)-0.5*phi_L(IRHOU:IRHOW)*PrimL(IB1)   !1/2 phi B_n
#elif NONCONS==3 /*Janhunen*/
          phi_L(IB1:IB3)=U_master(IRHOU:IRHOW,p,q,SideID)/U_master(IRHO1,p,q,SideID)
          Flux(IB1:IB3,p,q,SideID)=Flux(IB1:IB3,p,q,SideID)-0.5*phi_L(IB1:IB3)*PrimL(IB1)   !1/2 phi B_n
#endif /*NONCONSTYPE*/
#if defined (PP_GLM) && defined (PP_NC_GLM)
          ! galilean invariance for GLM
          Flux((/IRHOE,PP_nVar/),p,q,SideID)=Flux((/IRHOE,PP_nVar/),p,q,SideID)-0.5*PrimL(IU)*(/PrimL(IPSI)**2,PrimL(IPSI)/)  !1/2 v_n (psi^2,psi) 
