...
!===================================================================================================================================
  pure subroutine SensorsByFernandezEtAl(sensors,U,gradPx,gradPy,gradPz,Mh_inv)
    ...
    real :: h_eta         ! Element size in the direction of the magnetic pressure gratient
    ...
    gradPm(1) = dot_product( U(6:8), gradPx(6:8) )
    ...
    h_eta = 2. * norm2(gradPm) / sqrt( dot_product(gradPm,matmul(Mh_inv,gradPm)) + eps )  ! TODO: This is probably NOT the right metric tensor
    
    sensors(4) = h_eta * norm2(J) / (PP_N * 2. * sqrt(PP_Pi) * norm2 (U(6:8)) +eps)
