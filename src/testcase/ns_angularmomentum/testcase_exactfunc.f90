...
CASE(10001) !vortex with deplaced density bump,2d
  ...
  du     = A*exp(0.5*(1.0-dot_product(xi(1:2),xi(1:2))))
  vel(1) = - du *xi(2)
  vel(2) =   du *xi(1)
  vel(3) = 0.0
  du     = 3.0*A*exp(0.5*(1.0-dot_product((xi(1:2)-dx),(xi(1:2)-dx))))
...
CASE(10002) !vortex with deplaced density bump
  ...
  du     = A*exp(0.5*(1.0-dot_product(xi,xi)))
  vel(1) = - du *xi(2)
  vel(2) =   du *xi(1)
  vel(3) = 0.0
  du     = 3.0*A*exp(0.5*(1.0-dot_product((xi-dx),(xi-dx))))
  dT     = (KappaM1)*rho_0/(2.0*Kappa*p_0)*du*du
  rho    = (1.0-dT)**(sKappaM1)
  pres   = p_0*rho**(Kappa)
  rho    = rho_0*rho
