...
#include "defines.h"
!==================================================================================================================================
!> Module providing routines for reading Fluxo parameter files. 
!> 
...
IF(MPIROOT)THEN
  ...
  IF(filename(iExt+1:iExt+3) .NE. 'ini') THEN
    WRITE(UNIT_StdOut,*) "Usage: fluxo parameter.ini [restart.h5] [keyword arguments]"
    CALL Abort(__STAMP__,&
...
IF ((.NOT.markdown).AND.(LEN_TRIM(name).EQ.0)) THEN
  ...
  SWRITE(UNIT_StdOut,'(A)')    "! Default Parameter File generated using 'fluxo --help' "
