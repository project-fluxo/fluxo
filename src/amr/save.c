...
SaveMeshFillArray2_iter(p4est_iter_volume_info_t *info, void *user_data)
{
    ...
    for (iLocalSide = 0; iLocalSide < 6; iLocalSide++) // This is a HOPR Numeration
    {
        ...
        if (dataquad->nbElementID[iLocalSide][1] != 0)
        {
            ...
            SideInfo[(SIndex)*5 + (i - 1)] = fabs(dataquad->SidesID[iLocalSide]); // GlobalSideID positive (master) and negative (slave)
            ...
            {   
                ...
                i = 2;
                // MortarSides[6][4]
                SideInfo[(SIndex)*5 + (i - 1)] = fabs(dataquad->MortarSides[iLocalSide][iM]); // GlobalSideID positive (master) and negative (slave)
        ...
        { // Non hanging side
            ...
            if (dataquad->flips[iLocalSide] == 0) //Boundary Side
            {
                ...
                i = 2;
                // MortarSides[6][4]
                SideInfo[(SIndex)*5 + (i - 1)] = dataquad->SidesID[iLocalSide]; // GlobalSideID positive (master) and negative (slave)
            ...
            if (dataquad->flips[iLocalSide] < 0 && dataquad->SidesID[iLocalSide] < 0) //Small slave Mortar Side
            {
                ...
                SideInfo[(SIndex)*5 + (i - 1)] = -104; //Side Type = HexaHedron non-linear
                i = 4;
                SideInfo[(SIndex)*5 + (i - 1)] = ((int)fabs(dataquad->flips[iLocalSide])) % 10; //It is a Flip!     //fabs(dataquad->flips[iLocalSide]); // 10*nbLocSide + FLIP dataquad->
            }
            else
            {
                i = 1; //Non Mortar Side
                ...
                i = 4;
                SideInfo[(SIndex)*5 + (i - 1)] = fabs(dataquad->flips[iLocalSide]); //fabs(dataquad->flips[iLocalSide]);

                }
            i = 2;
            // MortarSides[6][4]
            if (dataquad->flips[iLocalSide] < 0)                                           // Slave
                SideInfo[(SIndex)*5 + (i - 1)] = -1 * fabs(dataquad->SidesID[iLocalSide]); // GlobalSideID positive (master) and negative (slave)
            else
                SideInfo[(SIndex)*5 + (i - 1)] = fabs(dataquad->SidesID[iLocalSide]); // GlobalSideID positive (master) and negative (slave)
...
p4est_savef_data_t *save_mesh(p4est_t *p4est1)
{
    ...
    savemesh_inner_data_t *ghost_data = (savemesh_inner_data_t *)malloc(ghost->ghosts.elem_count * sizeof(savemesh_inner_data_t));
